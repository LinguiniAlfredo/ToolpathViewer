...
public sealed class PathShapeTests
{
    ...
    [Fact]
    public void HFile_WhenParsedAsMachineInstructions_YieldsDirectToolpath_WithoutMetadataOrShapeContourPollution()
    {
        // Sample H-file machine code
        string sampleHCode = @"
; Toolpath generated by Ablation Studio - Box4mm.h
; Date: 2026-09-12 18:25:00 UTC
HCH 1 1 ;Layer 1
SL X0.0000 Y0.0000 Z0.0000 M05
SL X-2.0000 Y-2.0000 Z0.0000 M05
SL X-2.0000 Y-2.0000 Z0.0000 M03
SL X2.0000 Y-2.0000 Z0.0000 M03
SL X2.0000 Y2.0000 Z0.0000 M03
SL X-2.0000 Y2.0000 Z0.0000 M03
SL X-2.0000 Y-2.0000 Z0.0000 M03
SL X0.0000 Y0.0000 Z0.0000 M05
";

        // 1. Verify no metadata markers exist in raw exported H-file
        Assert.DoesNotContain("ABLATION_STUDIO_PROJECT_BEGIN", sampleHCode);
        Assert.DoesNotContain("ABLATION_STUDIO_PROJECT_END", sampleHCode);

        // 2. Parse as pure machine instructions for simulation/viewing
        Toolpath toolpath = ToolpathParser.ParseText(sampleHCode, "Box4mm.h");
        Assert.Equal(6, toolpath.Segments.Count);
        Assert.Equal(4, toolpath.Statistics.CutSegmentsCount);
        Assert.Equal(2, toolpath.Statistics.RapidSegmentsCount);
        Assert.False(toolpath.BoundingBox.IsEmpty);

        // 3. Document remains pure: machine H-file is loaded for simulation without polluting ShapeDocument
        var doc = new ShapeDocument();
